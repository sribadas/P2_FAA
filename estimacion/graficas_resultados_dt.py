...
OUTPUT_DIR = BASE_DIR / "salidas_dt"
DATA_DIR = OUTPUT_DIR / "datos"
TABLES_DIR = OUTPUT_DIR / "tablas"
FIGURES_DIR = OUTPUT_DIR / "graficas"
RESULTS_PATH = DATA_DIR / "resultados_dt.csv"
CONFUSION_PATH = DATA_DIR / "matrices_confusion_dt.csv"
TABLE_CSV_PATH = TABLES_DIR / "tabla_comparativa_dt.csv"
TABLE_MD_PATH = TABLES_DIR / "tabla_comparativa_dt.md"
FIG_RANKING_PATH = FIGURES_DIR / "fig_dt_ranking.png"
FIG_SCATTER_PATH = FIGURES_DIR / "fig_dt_f1_vs_error.png"
FIG_CONFUSION_PATH = FIGURES_DIR / "fig_dt_confusion_mejor.png"
FIG_TRAIN_TEST_PATH = FIGURES_DIR / "fig_dt_train_vs_test.png"


def clean_text(text: str) -> str:
    return str(text).replace("Â±", "±").strip()
...
def build_comparison_table(df: pd.DataFrame) -> pd.DataFrame:
    table = df.copy()
    table["MaxDepth"] = table["MaxDepth"].map(lambda v: str(int(v)))
    table["Train Acc ± std (%)"] = (
...
def plot_ranking(df: pd.DataFrame) -> None:
    ...
    ax.set_title("Comparativa de configuraciones DT")
...
def plot_f1_vs_error(df: pd.DataFrame) -> None:
    ...
    ax.scatter(x, y, s=240, c="#2a9d8f", alpha=0.8, edgecolors="black", linewidths=0.6)
    ...
    ax.set_title("Relacion entre error y F1 por configuracion DT")
...
def plot_train_vs_test(df: pd.DataFrame) -> None:
    ...
    ax.invert_yaxis()
    ax.set_xlabel("Porcentaje (%)")
    ax.set_title("Comparacion de accuracy en entrenamiento y prueba - DT")
    ax.grid(axis="x", linestyle="--", alpha=0.3)
    ax.legend()
    plt.tight_layout()
...
def plot_best_confusion(df: pd.DataFrame) -> None:
    ...
    ax.set_title(f"Matriz de confusion - Mejor configuracion DT: {best_name}")
...
def print_summary(table: pd.DataFrame) -> None:
    ...
    print(
        f" - {best['Nombre']} | Accuracy: {best['Acc ± std (%)']} | F1: {best['F1 ± std (%)']}"
    )
